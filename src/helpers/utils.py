...
from FBCNN.models.network_fbcnn import FBCNN
...
def get_scheduled_params(param, param_schedule, step_counter, ignore_schedule=False):
    ...
    if ignore_schedule is False:
        ...
        assert(len(vals) == len(steps) +
               1), f'Mispecified schedule! - {param_schedule}'
...
def setup_generic_signature(args, special_info):
    ...
    time_signature = '{:%Y_%m_%d_%H:%M}'.format(
    ...
    if args.name is not None:
        ...
        args.name = '{}_{}_{}'.format(
            args.dataset, special_info, time_signature)
...
def save_model(model, optimizers, mean_epoch_loss, epoch, device, args, logger, multigpu=False):
    ...
    metadata = dict(image_dims=args.image_dims,
                    epoch=epoch, steps=model.step_counter)
    args_d = dict((n, getattr(args, n))
                  for n in dir(args) if not (n.startswith('_') or 'logger' in n))
    ...
    model_path = os.path.join(directory, '{}_epoch{}_idx{}_{}.pt'.format(
        model_name, epoch, model.step_counter, timestamp))

    if os.path.exists(model_path):
        model_path = os.path.join(directory, '{}_epoch{}_idx{}_{:%Y_%m_%d_%H:%M:%S}.pt'.format(
    ...
    logger.info('Saved model at Epoch {}, step {} to {}'.format(
...
def load_model(save_path, logger, device, model_type=None, model_mode=None, current_args_d=None, prediction=True,
        ...
        if silent is False:
            for k, v in current_args_d.items():
                try:
                    ...
                    logger.warning(
                        'Argument {} (value {}) not present in recorded arguments. Using current argument.'.format(k, v))
                    continue

                if loaded_v != v:
                    logger.warning(
                        'Current argument {} (value {}) does not match recorded argument (value {}). Recorded argument will be overriden.'.format(k, v, loaded_v))
    ...
    logger.info("Estimated model size (under fp32): {:.3f} MB".format(
        count_parameters(model) * 4. / 10**6))
    ...
    if prediction is True:
        ...
        optimizers = dict(amort=amortization_opt,
                          hyper=hyperlatent_likelihood_opt)

        if model.use_discriminator is True:
            discriminator_parameters = model.Discriminator.parameters()
            disc_opt = torch.optim.Adam(
                discriminator_parameters, lr=args.learning_rate)
...
def load_enhancement_model(checkpoint_path: str, device: str = 'cuda', model_type: str = None):
    if not model_type or model_type == 'DnCNN':
        ...
        model = FBCNN(in_nc=3, out_nc=3, nc=[
                      64, 128, 256, 512], nb=4, act_mode='R')
        model.load_state_dict(torch.load(checkpoint_path))
...
def log_summaries(writer, storage, step, use_discriminator=False):
    ...
    gan_scalars = ['disc_loss', 'gen_loss',
                   'weighted_gen_loss', 'D_gen', 'D_real']
    ...
    for scalar in compression_scalars:
        writer.add_scalar('compression/{}'.format(scalar),
                          storage[scalar][-1], step)
    ...
    if use_discriminator is True:
        ...
        for scalar in gan_scalars:
            writer.add_scalar('GAN/{}'.format(scalar),
                              storage[scalar][-1], step)

    # Breakdown overall loss
    writer.add_scalars('compression_loss_breakdown',
                       compression_loss_breakdown, step)
...
def log(model, storage, epoch, idx, mean_epoch_loss, current_loss, best_loss, start_time, epoch_start_time,
    ...
    if writer is not None:
        log_summaries(writer, storage, model.step_counter,
