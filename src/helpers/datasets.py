...
class OpenImages(BaseDataset):
    ...
    """
    files = {"train": "train", "test": "valid", "val": "valid"}
...
class VisualGenome(BaseDataset):
    ...
    """
    files = {"train": "VG_100K", "test": "test", "val": "VG_100K"}

    def __init__(self, root, mode='train', crop_size=256, normalize=False, **kwargs):
        super().__init__(root, [transforms.ToTensor()], **kwargs)
        ...
        if mode == 'train':
            self.imgs = self.imgs[:int(len(self.imgs) * 0.7)]
        elif mode == 'validation':
            self.imgs = self.imgs[int(len(self.imgs) * 0.7):]
